...
class StockMove(models.Model):
    ...
    def _get_price_unit(self, move_line=None):
        ...
        self.ensure_one()
        price_unit = not self.company_id.currency_id.is_zero(self.price_unit) and self.price_unit or self.product_id.standard_price
        ...
        if self.origin_returned_move_id and self.origin_returned_move_id.sudo().stock_valuation_layer_ids:
            if move_line:
                svls = self.origin_returned_move_id.stock_valuation_layer_ids.filtered(lambda s: s.stock_move_line_id == move_line)
                if not svls:
                    svls = self.origin_returned_move_id.stock_valuation_layer_ids
            if svls:
