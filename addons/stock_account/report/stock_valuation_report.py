...
class StockValuationReport(models.AbstractModel):
    ...
    def _get_report_data(self, date=False, product_category=False, warehouse=False):
        ...
        lot_domain = [("product_id", "in", valued_products.ids)]
        lots = lot_model.search(lot_domain)
        # PERF: computing the inventory value walks the lot's moves, so it is only run on
        # lots still holding stock. The accounting value is read from the journal items and
        # is computed for every lot, otherwise a lot emptied but left with an unbalanced
        # valuation account would never show up on the report (same rationale as the
        # product-level split done upstream for products with no quantity).
        lot_ids_with_qty = set(lot_model.search(lot_domain + [("virtual_available", "!=", 0)]).ids)
